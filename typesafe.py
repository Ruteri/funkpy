...
def _get_check_list_args_fn(expected_class):
	if not hasattr(expected_class, '__args__'): return None
	...
	def check_args(obj):
		for items in obj.items():
			for index, item in enumerate(items):
				if not checks[index](argtypes[index], item):
					return False
		return True
	return check_args
...
class typesafe(object):
	...
	def __call__(self, *args, **kwargs):
		logger.log(Logger.DEBUG, '@typesafe: calling with', args, kwargs)
...
@typesafe
def testfn(a: str, b: int, c: typing.List[int], d: typing.Dict[str, int], l: typing.Callable, f: typing.Callable[[str], typing.Any]) -> int:
	return c[0]
...
class classtest():
	...
	@typesafe_mf
	def init2(self, arg1: str):
		pass

def stub_fn(arg: str) -> int:
	return 0

testfn('a', 2, [1], {}, lambda arg: 0, stub_fn)
